...
from global_graph.model_mf import MFGCN
...
def parse_args():
    ...
    parser.add_argument('--vocab_path', type=str, default='datachem/drug_list_zhang.csv', choices=['datachem/drug_list_deep.csv', 'datachem/drug_list_miner.csv', 'datachem/drug_list_deep.csv'])

    # training
    parser.add_argument('--learning_rate', type=float, default=0.001, help='Initial learning rate.')
    parser.add_argument('--epochs', type=int, default=170, help='Number of epochs to train.')  # 170
    ...
    parser.add_argument('--dropout', type=float, default=0.3, help='Dropout rate')  # 0.3 0.5
    ...
    parser.add_argument('--graph_encoder', type=str, default='ggnn', choices=['dmpnn', 'ggnn'])
    ...
    parser.add_argument('--input_features_size', type=int, default=0,
                        help='Number of input features dimension.')
    ...
    if args.vocab_path == 'datachem/drug_list_deep.csv':
        args.learning_rate = 0.0001  # 若数据集为deep,学习率为0.0001 初始0.001
...
def load_data(args: Namespace, filepath: str, smiles2idx: dict = None) \
    ...
    if args.separate_val_path is not None and args.separate_test_path is not None and ext == '.csv':
        """
        .csv file can only provide (node1, node2) edges        1. load vocab file        2. load edges       3. construct adj 
        """
        ...
        adj = sp.csr_matrix((data, (all_edges[:, 0], all_edges[:, 1])),  # 所有数据集的邻接矩阵
...
def main():
    ...
    num_features = args.hidden_size  # hidden layers in MPN 300
    features_nonzero = 0
    ...
    adj_norm = sparse_mx_to_torch_sparse_tensor(adj_norm)  # 稀疏矩阵转化为稀疏tensor
    ...
    model = MFGCN(args, num_features, features_nonzero,
                          dropout=args.dropout,  # 0.3
                          bias=args.bias,  # store_true
                          sparse=False
                         )
    print(model)
    if args.cuda:
    ...
    for epoch in range(args.epochs):
        ...
        preds, feat, embed, re_feat, re_embed, loss = model(features, adj_norm, adj_tensor, drug_nums)
        # preds, feat, embed, re_feat, re_embed, loss, emb = model(features, adj_norm, adj_tensor, drug_nums)
        print("preds:", preds)
        ...
        avg_cost = args.alpha_loss * BCEloss + args.beta_loss * KLloss + args.gamma_loss * loss
        # avg_cost = args.alpha_loss * BCEloss + args.beta_loss * KLloss

        roc_curr, ap_curr, f1_curr, acc_score = get_roc_score(
            model, features, adj_norm, adj_orig, adj_tensor, drug_nums, train_edges, train_edges_false
        )
...
if __name__ == '__main__':
    args.feature_list = ["target", "enzyme", "transporter"]
